...
public class HeartbeatManager extends Thread {
	...
	public void processRequest(Management mgmt) {
		...
		if (hd == null) {
			...
			logger.error("Unknown heartbeat received from node ", mgmt.getHeader().getOriginator()+ " Node ID:"+mgmt.getGraph().getFromNodeId());
			return;
		} else {
	...
	public class CloseHeartListener implements ChannelFutureListener {
		...
		public void operationComplete(ChannelFuture future) throws Exception {
			if (outgoingHB.containsValue(heart)) {
				...
				//Checking for leader drop on channel close to start new election.
				if(heart.getNodeId() == ElectionManager.getInstance().whoIsTheLeader()){
					logger.info("Leader Left");
					ElectionManager.getInstance().setElectionNull();
					ElectionManager.getInstance().setLeaderNodeNull();
					ElectionManager.getInstance().setState(poke.server.managers.ElectionManager.RState.Follower);
					logger.info("Leader Left: Check who is the leader.");
					ElectionManager.getInstance().askWhoIsTheLeader();
					synchronized (syncPt) {
						System.out.println("Starting election after leader left.");
